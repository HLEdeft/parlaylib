  ...
  if (n < 2)
    ...
    parlay::parallel_for(
        ...
        [&](size_t i) {
          ...
          parlay::parallel_for(
              2, n / prime + 1, [&](size_t j) { flags[prime * j] = false; }, 1000);
        },1);
...
typename Seq::value_type mcss(Seq const& A) {
  ...
  auto f = [&](auto a, auto b) {
    auto [aa, pa, sa, ta] = a;
    auto [ab, pb, sb, tb] = b;
    return std::make_tuple(std::max(aa, std::max(ab, sa + pb)), std::max(pa, ta + pb), std::max(sa + ab, sb), ta + tb);
  };
  auto S = parlay::delayed_seq<std::tuple<T, T, T, T>>(
      A.size(), [&](size_t i) { return std::make_tuple(A[i], A[i], A[i], A[i]); });
...
static void bench_mcss(benchmark::State& state) {
  ...
  parlay::parallel_for(0, n, [&](auto i) { a[i] = (i % 2 == 0 ? -1 : 1) * i; });
...
double integrate(size_t num_samples, double start, double end, F f) {
  ...
  auto samples = parlay::delayed_seq<double>(num_samples, [&](size_t i) { return f(start + delta / 2 + i * delta); });
  return delta * parlay::reduce(samples, parlay::addm<double>());
}

static void bench_integrate(benchmark::State& state) {
  size_t n = state.range(0);
  auto f = [&](double q) -> double { return pow(q, 2); };
